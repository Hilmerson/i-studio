...
- **Never add a `Co-Authored-By: Codex ...` line (or any AI co-author/attribution) to commit messages.** Commits are authored solely by the repo owner.
...
- **Recipient:** `office@i-studio.sk` (must be an existing Websupport mailbox with matching `-f` envelope sender, or the mail is silently discarded). Anti-spam layers: honeypot `web`, JS token `cas` (a weak heuristic against scrape-and-post bots, not proof of a browser — it also means the no-JS form renders but cannot submit; it gets a visible "enable JS or email us" page), per-IP rate limit (10 *sent* messages/hour, temp-dir file; rejected attempts don't count), invalid-UTF-8 guard, Cyrillic → reject. The link+no-diacritics heuristic and the keyword blocklist do **not** reject: the message is delivered with `[možný spam]` in the subject and a note at the top of the body (owner's decision: better to see spam than lose a lead). Every rejection is a visible page with a Back button, never silent for humans.
